package main
/*
	生成windows可执行的文件 .exe CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build qqls_ExcelHelperV2_1.go
...
const (
	CodeZNotFound = -1	// 未找到
	CodeZOrderXlsxFileRegularName = "orders_export" 	// 订单正则匹配字段
	CodeZExpressXlsxFileRegularName = "订单" 	// 货运单正则匹配字段
...
func main() {
	...
	CodeZOrderXlsxFileName := readSpecifiedOrderXlsxFile()	// 订单文件名
...
// 读取特定订单excel
func readSpecifiedOrderXlsxFile() string {
	return readSpecifiedXlsxFile(CodeZOrderXlsxFileRegularName)
}
